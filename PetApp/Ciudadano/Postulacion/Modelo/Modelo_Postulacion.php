...
class MetodoPostulacion{
	...
	$sql="select * from postulacion where cedu='to'";
		...
		public function  Insertarpostulacion($tel,$dir,$fij,$cpdf,$es,$re,$ub,$tv,$res,$rec,$ce,$ca){
		$conectar=new conectarproyecto();
		$conexion=$conectar->conexion();
		...
		if ($result->num_rows>0) {
			...
			$consulta=$conexion->prepare($sql);
			$consulta->bind_param('ssssiississi',$tel_p,$dir_p,$fij_p,$cpdf_p,$es_p,$re_p,$ub_p,$tv_p,$res_p,$rec_p,$ce_p,$ca_p);
			$tel_p=$tel;
			$dir_p=$dir;
			$fij_p=$fij;
			$cpdf_p=$cpdf;
